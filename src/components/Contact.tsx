...
const Contact = () => {
    return (
        <section id='contact' className='section'>
            <div className='container grid lg:grid-cols-2 lg:items-stretch reveal-up'>
                <div className='mb-12 lg:mb-0 lg:flex lg:flex-col'>
                    <h2 className='headline-2 max-w-[12ch]'>Contact me</h2>
                    ...
                    <div className='flex items-center gap-2 mt-auto'>
                        {socialLinks.map(({ icon, href }, index) => (
                            <Link to={href} className='h-12 w-12 grid place-items-center ring-2 ring-zinc-50/5 rounded-lg transition-[backgeound-color, color] hover:bg-zinc-50 hover:text-zinc-950 active:bg-zinc-50/80' target='_blank' key={index}>
                                {icon}
                            </Link>
                        ))
                        }
                ...
                <form action="https://getform.io/f/bdrrqzjb" method="post" >
                    <div className='md:grid md:items-center md:grid-cols-2 gap-2'>
                        <div className='mb-4'>
                            <label htmlFor="name" className='label reveal-up'>Name</label>
                            <input type="text" id="name" name="name" className='text-field reveal-up' placeholder='umrao bisht' autoComplete='name' required />
                        </div>
                        <div className='mb-4'>
                            <label htmlFor="email" className='label reveal-up'>Email</label>
                            <input type="email" id="email" name="email" className='text-field reveal-up' placeholder='umraobisht1@gmail.com' autoComplete='email' required />
                        </div>
                    </div>
                    <div className='mb-4'>
                        <label htmlFor="message" className='label reveal-up'>Message</label>
                        <textarea id="message" name="message" className='text-field resize-y min-h-32 max-h-80 reveal-up' placeholder='Type your message' required></textarea>
                    </div>
                    <button type="submit" className="[&]:max-w-full w-full justify-center text-white bg-gradient-to-r from-purple-500 via-purple-600 to-purple-700 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-purple-300 dark:focus:ring-purple-800 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 reveal-up">Submit</button>
                </form>
            </div>
        </section>
    )
